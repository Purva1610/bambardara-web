...
/**
 * Map frontend purpose values to backend ConcernType enum values
 */
function mapPurposeToConcernType(purpose) {
  const mapping = {
    BOOKING: 'BOOKING_ENQUIRY',
    STAY_HOSPITALITY: 'STAY_AND_HOSPITALITY',
    ACTIVITIES_ADVENTURE: 'ACTIVITIES_AND_ADVENTURE',
    FARM_NATURE_TRAILS: 'FARM_AND_NATURE_TOURS',
    PAYMENT_REFUND: 'PAYMENT_OR_REFUND',
    FEEDBACK_COMPLAINT: 'FEEDBACK_OR_COMPLAINT',
    INVESTMENT_PLANS: 'INVESTMENT',
...
/**
 * Collects all relevant details based on enquiry purpose
 */
function collectDetails(form) {
  const details = {};

  if (form.purpose === 'BOOKING' || form.purpose === 'STAY_HOSPITALITY') {
    details.arrival = form.arrival;
    details.departure = form.departure;
    details.guests = form.guests;
    details.roomPreference = form.roomPreference;
    details.specialRequests = form.specialRequests;
  } else if (form.purpose === 'ACTIVITIES_ADVENTURE') {
    details.activityType = form.activityType;
    details.activityDate = form.activityDate;
    details.experienceLevel = form.experienceLevel;
    details.guests = form.guests;
  } else if (form.purpose === 'FARM_NATURE_TRAILS') {
    details.trailType = form.trailType;
    details.activityDate = form.activityDate;
    details.groupSize = form.groupSize;
    details.guidePreference = form.guidePreference;
  } else if (form.purpose === 'PAYMENT_REFUND') {
    details.bookingReference = form.bookingReference;
    details.transactionId = form.transactionId;
    details.issueType = form.issueType;
    details.amount = form.amount;
  } else if (form.purpose === 'FEEDBACK_COMPLAINT') {
    details.stayDate = form.stayDate;
    details.roomVilla = form.roomVilla;
    details.feedbackType = form.feedbackType;
  } else if (form.purpose === 'INVESTMENT_PLANS') {
    details.capitalRange = form.capitalRange;
    details.investmentType = form.investmentType;
    details.contactPreference = form.contactPreference;
    details.timeline = form.timeline;
  } else if (form.purpose === 'MEMBERSHIP_PLANS') {
    details.membershipTier = form.membershipTier;
    details.startDate = form.startDate;
    details.contactPreference = form.contactPreference;
    details.timeline = form.timeline;
  } else if (form.purpose === 'OTHER') {
    details.subject = form.subject;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        purpose: mapPurposeToConcernType(form.purpose),
        message: form.message.trim(),
        details: collectDetails(form),
      };

      await apiRequest('/api/enquire', {
        method: 'POST',
              ...
              <Reveal>
                <div className="border border-stone bg-warm-sand px-8 py-12 md:px-12 md:py-16">
                  ...
                  <p className="mt-6 max-w-prose font-body text-[0.9rem] font-light leading-[1.9] text-light-charcoal">
                    Your enquiry has been received. One of the family will reply
                    within a day. For anything urgent, call{' '}
                    <a
