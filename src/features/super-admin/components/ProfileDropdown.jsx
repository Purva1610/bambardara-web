...
import { useTheme, ACCENT_OPTIONS } from '../../../app/providers/ThemeContext';
...
/**
 * Real profile menu for the Super Admin shell — unlike the CEO dashboard's
 * ProfileDropdown (hardcoded to a placeholder "AJ" / "CEO"), this one reads
 * the signed-in user from AuthContext and their business role from
 * useAccess() (GET /api/me/access), never a hardcoded value.
 */
export default function ProfileDropdown() {
  ...
  const roleLabel = role ? formatRoleLabel(role) : 'No role assigned';
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="hidden text-left sm:block">
          <span className="block max-w-[9rem] truncate text-xs font-medium leading-tight text-text">{displayName}</span>
          <span className="block text-[0.65rem] leading-tight text-muted">{roleLabel}</span>
        </span>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="p-2">
              <button onClick={() => setShowTheme((v) => !v)} className="menu-row" type="button">
            ...
            <div className="border-t border-line p-2">
              <button
                ...
                className="menu-row text-[#a24b3f]"
              >
