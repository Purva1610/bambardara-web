...
export default function FinanceInvestments() {
  ...
  /*
   * Cleanup any pending refresh timer when the component
   * unmounts.
   */
  useEffect(() => {
    return () => {
      // No persistent timer is stored here because refreshData
      // uses a short-lived browser timeout.
    };
  }, []);
  ...
  /* =======================================================
     ...
     IMPORTANT:
     jsPDF and jspdf-autotable are intentionally NOT imported
     at the top of this file.
     
     They are downloaded only when the user clicks
     "Download Report".
  ======================================================= */
  ...
  const generateReport = async () => {
    ...
    try {
      /*
       * Load both PDF packages only when required.
       *
       * Promise.all() allows both packages to load in parallel.
       */
      ...
      alert(
        "Unable to generate the report. Please check that jspdf and jspdf-autotable are installed."
      );
    } finally {
